 ...
 * <tt>LBFGSCP.java</tt> for more information.
 */
public class McsrchCP {
    private static int infoc[] = new int[1];
    private static double dginit = 0;
    private static double dgtest = 0;
    ...
    private static double stmin = 0;
    private static double stmax = 0;
    private static double width = 0;
    private static double width1 = 0;
    ...
    public static void mcsrch(int n, double[] x, double f, double[] g, double[] s, int is0, double[] stp, double ftol,
        ...
        if (info[0] != -1) {
            infoc[0] = 1;
            if (n <= 0 || stp[0] <= 0 || ftol < 0 || LBFGSCP.gtol < 0 || xtol < 0 || LBFGSCP.stpmin < 0 || LBFGSCP.stpmax < LBFGSCP.stpmin || maxfev <= 0)
                return;
            ...
            finit = f;
            dgtest = ftol * dginit;
            width = LBFGSCP.stpmax - LBFGSCP.stpmin;
            width1 = width / p5;
        ...
        while (true) {
            if (info[0] != -1) {
                ...
                stp[0] = Math.max(stp[0], LBFGSCP.stpmin);
                stp[0] = Math.min(stp[0], LBFGSCP.stpmax);
            ...
            if (stp[0] == LBFGSCP.stpmax && f <= ftest1 && dg <= dgtest) info[0] = 5;

            if (stp[0] == LBFGSCP.stpmin && (f > ftest1 || dg >= dgtest)) info[0] = 4;

            if (nfev[0] >= maxfev) info[0] = 3;

            if (brackt[0] && stmax - stmin <= xtol * stmax) info[0] = 2;

            if (f <= ftest1 && Math.abs(dg) <= LBFGSCP.gtol * (-dginit)) info[0] = 1;
            ...
            if (stage1 && f <= ftest1 && dg >= Math.min(ftol, LBFGSCP.gtol) * dginit) stage1 = false;
    ...
    public static void mcstep(double[] stx, double[] fx, double[] dx, double[] sty, double[] fy, double[] dy,
        ...
        if ((brackt[0] && (stp[0] <= Math.min(stx[0], sty[0]) || stp[0] >= Math
                .max(stx[0], sty[0]))) || dx[0] * (stp[0] - stx[0]) >= 0.0 || stpmax < stpmin) return;
