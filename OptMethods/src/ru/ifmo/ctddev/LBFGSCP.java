...
public class LBFGSCP {
    /**
     * Specialized exception class for LBFGSCP; contains the
     ...
     * optimization iterations before <tt>LBFGSCP.lbfgs</tt> automatically stops
     * (by reaching a very small gradient) should copy this vector instead
     * of using <tt>x</tt>. When <tt>LBFGSCP.lbfgs</tt> automatically stops,
     ...
     * function since the last time LBFGSCP was restarted. The total number of function
     ...
     *               <li><code>iflag = -3</code> Improper input parameters for LBFGSCP
     *               (<code>n</code> or <code>m</code> are not positive).
     *               </ul>
     * @throws LBFGSCP.ExceptionWithIflag
     */
    ...
    public static void lbfgs(int n, int m, double[] x, double f, double[] g, boolean diagco, double[] diag,
        ...
        if (iflag[0] == 0) {
            ...
            if (gtol <= 0.0001) {
                System.err.println("LBFGSCP.lbfgs: gtol is less than or equal to 0.0001. It has been reset to 0.9.");
                gtol = 0.9;
            }
                ...
                for (i = 1; i <= n; i += 1) {
                    if (diag[i - 1] <= 0) {
                        ...
                        throw new ExceptionWithIflag(iflag[0],
                                "The " + i + "-th diagonal element of the inverse hessian approximation is not positive.");
                    }
                }
            } else {
                for (i = 1; i <= n; i += 1) {
        ...
        while (true) {
            ...
            McsrchCP.mcsrch(n, x, f, g, w, ispt + point * n, stp, ftol, xtol, maxfev, info, nfev, diag);
            ...
            if (info[0] != 1) {
                iflag[0] = -1;
                throw new ExceptionWithIflag(iflag[0],
                        "Line search failed. See documentation of routine mcsrch. Error return of line search: info = " + info[0] + " Possible causes: function or gradient are incorrect, or incorrect tolerances.");
            }
